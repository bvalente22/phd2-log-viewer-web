...
const beforeLast = (s, ch) => {
    const i = s.lastIndexOf(ch);
    return i < 0 ? s : s.slice(0, i);
};
export function addInfo(infos, idx, raw) {
    let info = raw;
    if (info.startsWith(SETTLING_PFX))
        info = info.slice(SETTLING_PFX.length);
    else if (info.startsWith(PARAM_PFX))
        info = info.slice(PARAM_PFX.length);
