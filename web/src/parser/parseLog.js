import { GUIDING_BEGINS, GUIDING_HEADING, GUIDING_ENDS, CALIBRATION_BEGINS, CALIBRATION_HEADING, CALIBRATION_ENDS, VERSION_PREFIX, MOUNT_KEY, AO_KEY, PX_SCALE, XALGO, YALGO, MINMOVE, INFO_KEY, startsWith, isEmpty, rtrim, getDbl, starWasFound, } from './tokens';
...
const getMinMo = (ln, lim) => {
    lim.minMo = getDbl(ln, MINMOVE, 0);
};
const parseIsoCombined = (s) => {
...
export function parseLog(text) {
    ...
    for (const sec of log.sections) {
        if (sec.type === 'GUIDING')
            fixupNonMonotonic(log.sessions[sec.idx]);
    }
    return log;
}
