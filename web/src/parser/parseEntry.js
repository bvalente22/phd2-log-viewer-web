...
export function parseEntry(ln) {
    ...
    if ([dx, dy, raraw, decraw, raguide, decguide].some(Number.isNaN))
        return null;
    let radur = cols[9] === '' ? 0 : (() => {
        const v = parseInt(cols[9], 10);
        return Number.isFinite(v) ? v : NaN;
    })();
    if (Number.isNaN(radur))
        return null;
    const raDir = cols[10];
    if (raDir) {
        if (raDir[0] === 'E') {
    ...
    if ([mass, snr, err].some((v) => !Number.isFinite(v)))
        return null;
    let info = '';
